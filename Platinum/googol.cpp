...
BigInteger solve2(BigInteger u, BigInteger val) { //node u = val, or u = val + 1
	if (u == 0) return 0;
	cout << u << endl;
	string a, b; cin >> a >> b;
	BigInteger l(a), r(b);
	if (val % 2 == 1) {
		BigInteger right = (val - 1) / 2;
		BigInteger left = solve2(l, right);
		return 1 + left + right;
	}
	else {
		BigInteger left = val / 2;
		BigInteger right = solve2(r, left - 1);
		return 1 + left + right;
	}
}

BigInteger solve1(BigInteger u) { //current node u
	if (u == 0) return 0;
	cout << u << endl;
	string a, b; cin >> a >> b;
	BigInteger l(a), r(b);
	BigInteger right = solve1(r);
	BigInteger left = solve2(l, right);
	return 1 + left + right;
}
